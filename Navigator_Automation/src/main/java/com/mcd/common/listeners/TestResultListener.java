package com.mcd.common.listeners;
...
import com.mcd.navigator.pages.Common;
...
public class TestResultListener extends TestListenerAdapter {
	...
	public void onTestSuccess(ITestResult tr) {
		...
		String browserName = tr.getMethod().getXmlTest()
				.getParameter("browser");
		try {
			if (browserName.equalsIgnoreCase("FIREFOX")) {
				createExcelReport(
						tr.getMethod().getXmlTest().getParameter("browser"),
						noOfTestsExecutedFF);
				noOfTestsExecutedFF++;
		...
		enterTestCaseResult(annotaionMethod.testCaseID(),
				annotaionMethod.testCaseDescription(), "Pass", "", "", tr
						.getMethod().getXmlTest().getParameter("browser"));
	}
	...
	public void onTestFailure(ITestResult tr) {
		...
		TestInfo annotationMethod = tr.getMethod().getConstructorOrMethod()
				.getMethod().getAnnotation(TestInfo.class);
		enterTestCaseResult(annotationMethod.testCaseID(),
				annotationMethod.testCaseDescription(), "Fail",
				testFailureReasonPartial, "file:///" + fileName, tr.getMethod()
						.getXmlTest().getParameter("browser"));
	...
	public void onTestSkipped(ITestResult tr) {
		...
		TestInfo annotationMethod = tr.getMethod().getConstructorOrMethod()
				.getMethod().getAnnotation(TestInfo.class);
		enterTestCaseResult(annotationMethod.testCaseID(),
				annotationMethod.testCaseDescription(), "Skipped",
				"Configuration failure", "", tr.getMethod().getXmlTest()
						.getParameter("browser"));
	...
	public String takeSnapShot(String tcName, ITestResult result)
		...
		File screenShotsFolder = new File(System.getProperty("user.dir")
				+ "//ScreenShots");
	...
	private void createExcelReport(String browser, int noOfTestsExecuted)
			throws IOException {
		File excelFolder = new File(System.getProperty("user.dir")
				+ "//ExcelReports");
		// Checking whether ExcelReports folder exists
		...
		if (!exlFile.exists()) {
			...
			Label failureReasonLabel = new Label(5, 0, "Failure Reason",
					cellFormat);
	...
	private void enterTestCaseResult(String testCaseID,
		...
		try {
			...
			workbookCopy = Workbook.createWorkbook(
					new File(System.getProperty("user.dir")
							+ "//ExcelReports//Automation_Report_" + browser
							+ "_" + getCurrentDate() + ".xls"), workbook);
			workbook.close();
			tempFile.delete();
			workbookCopy.write();
			workbookCopy.close();
